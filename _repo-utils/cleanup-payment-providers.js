    ...
    try {
      ...
      entries.forEach(function handleEntry(entry) {
        ...
        if (onlyName !== 'index' && !config.paymentProviders.includes(onlyName)) {
          ...
          console.info('Removed', `${dirPath}/${entry}`)
        }
      })
    } catch (e) {
      console.error('Skipping', dirPath)
    }
  })

  // Remove unused content of files
  ;['../src/page-components/checkout/payment/index.js'].forEach(function cleanupFile(filePath) {
    ...
    contents = handlebars.compile(contents)(
      config.paymentProviders.reduce((acc, p) => ({ ...acc, ['payment-provider-' + p]: true }), {}),
    )
